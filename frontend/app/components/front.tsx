  ...
  return (
    <div style={{ maxWidth: 400, margin: '40px auto', padding: 24, border: '1px solid #eee', borderRadius: 8, boxShadow: '0 2px 8px #eee' }}>
      ...
      <div
        ...
        style={{ border: '2px dashed #aaa', borderRadius: 8, padding: 32, textAlign: 'center', marginBottom: 16, background: '#fafafa' }}
      >
        {previewUrl ? (
          <img src={previewUrl} alt="Preview" style={{ maxWidth: '100%', maxHeight: 200, marginBottom: 8 }} />
        ) : (
        ...
        />
        <label htmlFor="fileInput" style={{ display: 'block', marginTop: 12, cursor: 'pointer', color: '#1976d2' }}>
      ...
      <button
        ...
        style={{ width: '100%', padding: 12, background: '#1976d2', color: '#fff', border: 'none', borderRadius: 4, fontWeight: 600, cursor: selectedFile ? 'pointer' : 'not-allowed', marginBottom: 12 }}
      >
        Upload & Verify
      </button>
      {uploadStatus && <div style={{ marginBottom: 8 }}>{uploadStatus}</div>}
      {result && <div style={{ padding: 12, background: '#e3f2fd', borderRadius: 4 }}>{result}</div>}
    </div>
  );
}